...
impl Port {
    ...
    async fn open(&self) -> Result<WriteStream, Error> {
        let name = env::var(self.name).ok_or_else(|| Error::from(format!("${} unset", self.name)))?;
        fs::open_append(&name).await
    }

    async fn with<'a, T, F: FnOnce(&'a mut WriteStream) -> T>(&'a mut self, f: F) -> Result<T, Error> {
        self.init.get_or_try_init(async {
            ...
            if let Ok(mut g) = self.lock.lock() {
                *g = Some(r);
                ...
                Err(Error::from(format!("failed to acquiring lock for ${} file handle", self.name)))
            }
        }).await?;
